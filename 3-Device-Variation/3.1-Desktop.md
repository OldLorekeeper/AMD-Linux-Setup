...
## 3.1.1 - Jellyfin Transcoding Settings

```
>>> Hardware acceleration: VAAPI
>>> VAAPI Device: /dev/dri/renderD128
>>> Select all hardware decoding options
>>> Enable hardware encoding
>>> Allow encoding in HEVC and AV1 format
>>> Transcode path: /dev/shm/jellyfin
>>> Audio boost when downmixing: 1
>>> Stereo downmix algorithm: Dave750
>>> Max muxing queue size: 8192
>>> Encoding preset: auto
```

<br/>
## 3.1.2 - Byparr Installation
...
The necessary `MONITOR`, `STREAM_MODE` and `DEFAULT_MODE` elements for custom Sunshine scripts symlinked to `/usr/local/bin/` should have been configured by `setup_desktop.zsh`.
