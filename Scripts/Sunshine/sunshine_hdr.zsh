...
case "$1" in
    enable)
        ...
        kscreen-doctor output.$MONITOR.mode.$STREAM_MODE output.$MONITOR.hdr.enable output.$MONITOR.wcg.enable output.$MONITOR.scale.1.75
        print -P "Status: %F{green}HDR Enabled (Scale 1.75)%f"
        ;;
    disable)
