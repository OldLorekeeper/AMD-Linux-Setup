#!/bin/zsh
# ------------------------------------------------------------------------------
# 6. Utility. Sunshine Resolution Switcher (Dell)
# Toggles resolution to 1080p for remote Dell XPS clients.
# ------------------------------------------------------------------------------

# region Init
setopt ERR_EXIT NO_UNSET PIPE_FAIL EXTENDED_GLOB
SCRIPT_DIR=${0:a:h}
print -P "\n%K{green}%F{black} STARTING DELL RES SWITCHER %k%f\n"
# endregion
...
case "$1" in
    enable)
        ...
        print -P "Status: %F{green}Dell Mode Enabled (1080p)%f"
        ;;
    disable)
        kscreen-doctor output.$MONITOR.mode.$DEFAULT_MODE output.$MONITOR.scale.1.0
        print -P "Status: %F{green}Dell Mode Disabled (Reverted to Native)%f"
        ;;
    *)
