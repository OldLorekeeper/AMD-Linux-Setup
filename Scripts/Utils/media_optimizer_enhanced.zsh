#!/usr/bin/env zsh
# media_optimizer_enhanced.zsh
# Optimizes media library using AMD GPU hardware encoding (HEVC/VAAPI).
# Targets ANY video format that is not already HEVC or AV1.
...
local -a files
files=("${(@f)$(find "$MEDIA_DIR" -type f \( -iname '*.mkv' -o -iname '*.mp4' -o -iname '*.avi' -o -iname '*.ts' -o -iname '*.webm' -o -iname '*.m4v' -o -iname '*.wmv' \))}")
...
for file in "${files[@]}"; do
    ...
    if [[ "$codec" != "hevc" && "$codec" != "av1" && -n "$codec" ]]; then
