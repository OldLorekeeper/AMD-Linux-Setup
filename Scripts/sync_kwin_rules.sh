...
{
  printf "# SIZES:\n# Small: %s\n# Tall: %s\n# Wide: %s\n\n" "$SMALL_SIZE" "$TALL_SIZE" "$WIDE_SIZE"
  cat "$GENERATED.tmp"
  # Strip only the size header lines from the template when copying unique rules
  grep -vE '^# *SIZES:|^# *Small:|^# *Tall:|^# *Wide:' "$TEMPLATE" | grep -vE '^[[:space:]]*$'
} > "$GENERATED"
