...
```
[Unit]
Description=Update Arch mirrorlist using Reflector
Wants=network-online.target
After=network-online.target

[Service]
Type=oneshot
ExecStart=/usr/bin/reflector --ipv4 -c GB -p https --download-timeout 5 --age 8 --latest 30 --fastest 10 --sort rate --save /etc/pacman.d/mirrorlist
Nice=19
...
```
[Unit]
Description=Run reflector-update 6h after boot, then every 6h

[Timer]
# First run occurs 6 hours after boot
OnBootSec=6h
# Subsequent runs every 6 hours after the last run
OnUnitActiveSec=6h
Persistent=true
