...
# Use config.ini in /opt/soularr. If your file is config.local.ini, either rename it to config.ini,
# or add:  ExecStart=/usr/bin/python /opt/soularr/soularr.py --config-dir /opt/soularr
ExecStart=/usr/bin/python /opt/soularr/soularr.py --config-dir /opt/soularr
...
```
systemctl list-timers | grep soularr
sudo journalctl -u soularr.service -n 50 --no-pager
