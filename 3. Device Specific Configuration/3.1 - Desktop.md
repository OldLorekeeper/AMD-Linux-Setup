...
```
sudo mkdir -p /opt/soularr/config && sudo cp /opt/soularr/config.ini /opt/soularr/config/config.ini && kate /opt/soularr/config/config.ini
```

Paste following over Lidarr and Slskd sections of new config.ini:
...
```
/usr/bin/python /opt/soularr/soularr.py --config-dir /opt/soularr/config --no-lock-file
```
