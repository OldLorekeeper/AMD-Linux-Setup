## Laptop 2026-01-19

### Clean install first version of setup_install.zsh

OS: Arch Linux x86_64  
 Host: EXCALIBUR-16-AMD7 (1.0)  
 Kernel: Linux 6.18.6-2-cachyos  
 Uptime: 1 hour, 40 mins  
 Packages: 1218 (pacman)  
 Shell: bash 5.3.9  
 Display (BOE0B38): 2560x1600 @ 1.25x in 16", 165 Hz [Built-in]  
 DE: KDE Plasma 6.5.5  
 WM: KWin (Wayland)  
 WM Theme: Darkly  
 Theme: Darkly (BreezeDark) [Qt], Breeze-Dark [GTK2], Breeze [GTK3/4]  
 Icons: Papirus-Dark [Qt], Papirus-Dark [GTK2/3/4]  
 Font: Adwaita Sans (10pt) [Qt], Adwaita Sans (10pt) [GTK2/3/4]  
 Cursor: breeze (24px)  
 Terminal: MainThread  
 CPU: AMD Ryzen 7 7840HS (16) @ 5.14 GHz  
 GPU: AMD Radeon 780M Graphics [Integrated]  
 Memory: 4.39 GiB / 30.66 GiB (14%)  
 Swap: 0 B / 15.33 GiB (0%)  
 Disk (/): 10.05 GiB / 952.87 GiB (1%) - btrfs  
 Local IP (wlan0): 192.168.86.44/24  
 Battery (Smart-BAT): 99% [AC Connected]  
 Locale: en_GB.UTF-8